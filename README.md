...
- lock 排他制御（ロック）のサンプル
  - no_lock.php 排他制御なしの例。SELECT〜UPDATE の間にロックを取らないため、複数プロセスが同じ値を読み取り加算が消える（ロストアップデート）排他制御をしていないので、別ウィンドウで同時に実行してもupdate文が１回しか実行されない

  - with_lock.php SELECT ... FOR UPDATE による排他制御ありの例。トランザクション内で行ロックを取得するため、複数プロセスでも正確な値が保たれる。排他制御をしているので、別ウィンドウで同時に実行してもupdate文が２回実行される
  - deadlock.php 2つのプロセスが2つの行を逆順でロックすることでデッドロックが発生する例 別ウィンドウでそれぞれ逆順のロックを実行
  - lock_timeout.php ロックを長時間保持し続けることで Lock wait timeout exceeded が発生する例。プロセスAがロックを30秒保持中にプロセスBが同じ行をロックしようとすると5秒でタイムアウトする
